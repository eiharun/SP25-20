...
// $Id: RH_RF95_CH.h,v 1 2025/04/05 eiharun
#ifndef RH_RF95_CH_h
#define RH_RF95_CH_h
...
/////////////////////////////////////////////////////////////////////
/// \class RH_RF95_CH RH_RF95_CH.h <RH_RF95_CH.h>
/// \brief Modified RH_RF95 Driver to send and receive unaddressed, unreliable datagrams via a LoRa 
/// capable radio transceiver. With custom headers.
///
...
class RH_RF95_CH : public RHSPIDriver
{
public:
    ...
    RH_RF95_CH(uint8_t slaveSelectPin = SS, uint8_t interruptPin = 2, RHGenericSPI& spi = hardware_spi);
    ...
    // Set Headers 
    // Overwritten header functions
    void setHeaderTo(uint8_t to) override { _txHeaderSeq = to; }
    void setHeaderFrom(uint8_t from) override { _txHeaderAck = from; }
    void setHeaderId(uint8_t id) override { _txHeaderCMD = id; }
    void setHeaderFlags(uint8_t set, uint8_t clear) override { _txHeaderLen = set; }

    uint8_t headerTo() override { return _rxHeaderSeq; }
    uint8_t headerFrom() override { return _rxHeaderAck; }
    uint8_t headerId() override { return _rxHeaderCMD; }
    uint8_t headerFlags() override { return _rxHeaderLen; }

    // Set Custom Headers
    void setHeaders(uint8_t seq, uint8_t ack, uint8_t cmd, uint8_t len){
        _txHeaderSeq = seq;
        _txHeaderAck = ack;
        _txHeaderCMD = cmd;
        _txHeaderLen = len;
    ...
    void setHeaderSeq(uint8_t seq) { _txHeaderSeq = seq; };
    void setHeaderAck(uint8_t ack) { _txHeaderAck = ack; };
    void setHeaderCMD(uint8_t cmd) { _txHeaderCMD = cmd; };
    void setHeaderLen(uint8_t len) { _txHeaderLen = len; };

    // Get Individual Recieved Headers
    uint8_t headerSeq() { return _rxHeaderSeq; }
    uint8_t headerAck() { return _rxHeaderAck; }
    uint8_t headerCMD() { return _rxHeaderCMD; }
    uint8_t headerLen() { return _rxHeaderLen; }

    // Get All Recieved Headers
    void getHeaders(uint8_t* seq, uint8_t* ack, uint8_t* cmd, uint8_t* len) {
        *seq = _rxHeaderSeq;
        *ack = _rxHeaderAck;
        *cmd = _rxHeaderCMD;
        *len = _rxHeaderLen;
...
protected:
    ...
    volatile uint8_t    _rxHeaderSeq;
    volatile uint8_t    _rxHeaderAck;
    volatile uint8_t    _rxHeaderCMD;
    volatile uint8_t    _rxHeaderLen; 

    // Transmitting packet headers
    uint8_t             _txHeaderSeq;
    uint8_t             _txHeaderAck;
    uint8_t             _txHeaderCMD;
    uint8_t             _txHeaderLen;
...
private:
    ...
    static RH_RF95_CH*     _deviceForInterrupt[];
