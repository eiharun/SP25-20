// RH_RF95_CH.cpp
//
// Author: Harun Adiyaman (haruna@vt.edu)
// $Id: RH_RF95_CH.cpp,v 1 2025/04/05 eiharun
...
RH_RF95_CH* RH_RF95_CH::_deviceForInterrupt[RH_RF95_NUM_INTERRUPTS] = {0, 0, 0};

// These are indexed by the values of ModemConfigChoice
// Stored in flash (program) memory to save SRAM
PROGMEM static const RH_RF95_CH::ModemConfig MODEM_CONFIG_TABLE[] =
{
    //  1d,     1e,      26
...
RH_RF95_CH::RH_RF95_CH(uint8_t slaveSelectPin, uint8_t interruptPin, RHGenericSPI& spi)
    :
...
bool RH_RF95_CH::setupInterruptHandler()
{
...
void RH_RF95_CH::handleInterrupt()
{
...
void RH_INTERRUPT_ATTR RH_RF95_CH::isr0()
{
    if (_deviceForInterrupt[0])
	_deviceForInterrupt[0]->handleInterrupt();
}
void RH_INTERRUPT_ATTR RH_RF95_CH::isr1()
{
    if (_deviceForInterrupt[1])
	_deviceForInterrupt[1]->handleInterrupt();
}
void RH_INTERRUPT_ATTR RH_RF95_CH::isr2()
{
...
bool RH_RF95_CH::recv(uint8_t* buf, uint8_t* len)
{
    if (!available())
	return false;
...
bool RH_RF95_CH::send(const uint8_t* data, uint8_t len)
{
    ...
    // The headers
    spiWrite(RH_RF95_REG_00_FIFO, _txHeaderSeq);
    spiWrite(RH_RF95_REG_00_FIFO, _txHeaderAck);
    spiWrite(RH_RF95_REG_00_FIFO, _txHeaderCMD);
    spiWrite(RH_RF95_REG_00_FIFO, _txHeaderLen);
    // The message data
...
bool RH_RF95_CH::printRegisters()
{
...
uint8_t RH_RF95_CH::maxMessageLength()
{
    return RH_RF95_MAX_MESSAGE_LEN;
}

bool RH_RF95_CH::setFrequency(float centre)
{
    // Frf = FRF / FSTEP
...
void RH_RF95_CH::setTxPower(int8_t power, bool useRFO)
{
    _useRFO = useRFO;
...
void RH_RF95_CH::setModemRegisters(const ModemConfig* config)
{
...
bool RH_RF95_CH::setModemConfig(ModemConfigChoice index)
{
    if (index > (signed int)(sizeof(MODEM_CONFIG_TABLE) / sizeof(ModemConfig)))
        return false;

    ModemConfig cfg;
    memcpy_P(&cfg, &MODEM_CONFIG_TABLE[index], sizeof(RH_RF95_CH::ModemConfig));
    setModemRegisters(&cfg);

    return true;
}

void RH_RF95_CH::setPreambleLength(uint16_t bytes)
{
    spiWrite(RH_RF95_REG_20_PREAMBLE_MSB, bytes >> 8);
    spiWrite(RH_RF95_REG_21_PREAMBLE_LSB, bytes & 0xff);
}

bool RH_RF95_CH::isChannelActive()
{
    // Set mode RHModeCad
    if (_mode != RHModeCad)
    {
...
void RH_RF95_CH::enableTCXO(bool on)
{
    if (on)
    {
...
int RH_RF95_CH::lastSNR()
{
 ...
 void RH_RF95_CH::setSpreadingFactor(uint8_t sf)
 {
   if (sf <= 6) 
...
void RH_RF95_CH::setSignalBandwidth(long sbw)
{
...
void RH_RF95_CH::setCodingRate4(uint8_t denominator)
{
...
void RH_RF95_CH::setLowDatarate()
{
...
void RH_RF95_CH::setPayloadCRC(bool on)
{
...
uint8_t RH_RF95_CH::getDeviceVersion()
{
